...
<template>
  <nav
    class="fixed top-7 left-1/2 -translate-x-1/2 w-[90%] md:w-[70%] backdrop-blur-sm bg-white/50 border border-gray-200 rounded-2xl px-6 py-6 z-50 shadow-lg will-change-[backdrop-filter] transform-gpu"
  >
    <div class="flex items-center justify-between">
      <RouterLink to="/" class="text-2xl md:text-3xl font-bold">
        <img :src="logo" alt="LinkKit logo" class="h-6 md:h-8" />
      </RouterLink>
      ...
      <div class="hidden md:flex items-center space-x-4 text-xl font-medium">
        ...
        <div v-else class="relative">
          <button
            ...
            class="w-10 h-10 rounded-full bg-green-400 text-white font-semibold hover:opacity-80 transition-opacity duration-300 flex items-center justify-center"
          >
            {{ userInitials }}
          </button>
      ...
      <div class="md:hidden flex items-center gap-3">
        ...
        <button
          v-if="authStore.isAuthenticated"
          class="w-8 h-8 rounded-full bg-green-400 text-white text-sm font-semibold flex items-center justify-center"
        >
          {{ userInitials }}
        </button>
    ...
    >
      ...
      >
        <RouterLink @click="isOpen = false" to="/about" class="py-2 transition-colors"
          >About</RouterLink
        >
        <RouterLink @click="isOpen = false" to="/tools" class="py-2 transition-colors"
          >Tools</RouterLink
        >

        <template v-if="authStore.isAuthenticated">
          <RouterLink @click="isOpen = false" to="/dashboard" class="py-2 transition-colors"
        ...
        <template v-else>
          <RouterLink @click="isOpen = false" to="/login" class="py-2 transition-colors"
            >Login</RouterLink
          >
          <RouterLink
            @click="isOpen = false"
